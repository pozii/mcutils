...
export const MINECRAFT_COLORS: Record<string, McColor> = {
...
export const MINECRAFT_FORMATS: Record<string, { code: string; name: string }> = {
...
export function motdToHtml(text: string): string {
  ...
  for (const seg of segments) {
    ...
    if (match) {
      ...
      else if (MINECRAFT_COLORS[code]) color = MINECRAFT_COLORS[code].hex
      continue
    }
    if (!seg) continue
...
export function getColorName(code: string): string | undefined {
  return MINECRAFT_COLORS[code]?.name
}

export function getColorRgb(code: string): [number, number, number] | undefined {
  return MINECRAFT_COLORS[code]?.rgb
}

export function colorCodeToHex(code: string): string | undefined {
