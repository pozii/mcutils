...
import { ServerInfo, PlayerSample, SERVER_PORT_DEFAULT, PingError } from '@mcutils/core'
...
export async function pingServer(options: PingOptions): Promise<ServerInfo> {
...
function createHandshakePacket(host: string, port: number, protocol: number): Buffer {
...
function createPingRequest(): Buffer {
...
function readVarInt(buffer: Buffer, offset: number): { value: number; size: number } {
  ...
  while (true) {
    ...
    size++
    if (size > 5) throw new PingError('VarInt too big')
    if (!(byte & 0x80)) break
  }
  return { value, size }
}

function prependPacketLength(buffer: Buffer): Buffer {
...
function parsePingResponse(data: Buffer, startTime: number): ServerInfo {
  if (data.length === 0) throw new PingError('Empty response from server')
  let offset = 0
  const { value: _packetLen, size: lenSize } = readVarInt(data, offset)
  offset += lenSize
  if (offset >= data.length) throw new PingError('Invalid packet: missing packet ID')
  const packetId = data[offset++]
  if (packetId !== 0x00) throw new PingError(`Unexpected packet ID: ${packetId}`)
  ...
  return {
    ...
    players: {
      ...
      sample: parsed.players?.sample?.map((p: { name: string; id: string }) => ({
        name: p.name,
        id: p.id,
      })) ?? [],
    },
...
function stripJsonDescription(desc: string | { text?: string; extra?: unknown[] } | unknown[]): string {
  if (typeof desc === 'string') return desc
  if (Array.isArray(desc)) {
    return desc.map((c: unknown) => {
  ...
  if (desc && typeof desc === 'object') {
    ...
    if (typeof d.text === 'string') {
      ...
      if (Array.isArray(d.extra)) {
        text += d.extra.map((e: unknown) => {
          if (typeof e === 'string') return e
          if (e && typeof e === 'object' && 'text' in (e as Record<string, unknown>)) return String((e as Record<string, unknown>).text ?? '')
          return ''
        }).join('')
      }
      return text
...
function jsonDescriptionToHtml(desc: string | { text?: string; extra?: unknown[]; color?: string; bold?: boolean; italic?: boolean; underlined?: boolean; strikethrough?: boolean } | unknown[]): string {
  if (typeof desc === 'string') return desc.replace(/</g, '&lt;').replace(/>/g, '&gt;')
  if (Array.isArray(desc)) {
    return desc.map((c: unknown) => {
      if (typeof c === 'string') return c.replace(/</g, '&lt;').replace(/>/g, '&gt;')
      if (c && typeof c === 'object') return renderJsonComponent(c as Record<string, unknown>)
      return ''
    }).join('')
  }
  if (desc && typeof desc === 'object') {
    return renderJsonComponent(desc as Record<string, unknown>)
  }
  return String(desc)
}

function renderJsonComponent(comp: Record<string, unknown>): string {
  ...
  if (comp.underlined) styles.push('text-decoration: underline')
  if (comp.strikethrough) styles.push('text-decoration: line-through')
  if (styles.length > 0) {
    text = `<span style="${styles.join('; ')}">${text}</span>`
  }
  if (Array.isArray(comp.extra)) {
    text += comp.extra.map((e: unknown) => {
      if (typeof e === 'string') return e.replace(/</g, '&lt;').replace(/>/g, '&gt;')
      if (e && typeof e === 'object') return renderJsonComponent(e as Record<string, unknown>)
      return ''
    }).join('')
  }
  return text
