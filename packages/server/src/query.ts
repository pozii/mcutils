...
export async function queryServer(host: string, port: number = SERVER_PORT_DEFAULT, timeout = 5000, full = false): Promise<QueryResult> {
...
function generateChallengeToken(): Buffer {
  const buf = Buffer.alloc(4)
  buf.writeInt32BE(Math.floor(Math.random() * 0x7FFFFFFF), 0)
  return buf
}

function createStatRequest(challengeToken: number, full: boolean): Buffer {
  ...
  buf[11] = full ? 0x00 : undefined as any
  return full ? Buffer.concat([buf, Buffer.alloc(1)]) : buf
}

function parseStatResponse(msg: Buffer, full: boolean): QueryResult {
  let offset = 5
  ...
  if (!full) {
    ...
    if (kvEnd === -1) throw new PingError('Invalid full stat response')
...
function readKvPairs(msg: Buffer, start: number): Record<string, string> {
...
function readKvPairsRaw(data: Buffer): Record<string, string> {
  const pairs: Record<string, string> = {}
