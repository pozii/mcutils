...
export class WorldBackup {
  static async create(options: BackupOptions): Promise<string> {
    ...
    try {
      ...
      return new Promise<string>((resolve, reject) => {
        ...
        archive.on('error', (e: Error) => reject(new McutilsError(e.message)))
  ...
  static async restore(backupFile: string, outputDir: string): Promise<void> {
  ...
  static async listBackups(backupDir: string): Promise<{ file: string; size: number; date: Date }[]> {
    const files = await fsPromises.readdir(backupDir)
    const results: { file: string; size: number; date: Date }[] = []
