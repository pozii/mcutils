function jsonOut(data: unknown, jsonFlag: boolean): void {
  if (jsonFlag) { console.log(JSON.stringify(data, null, 2)); return }
}

function label(k: string, v: string): string { return `${k}: ${v}` }
...
export async function cli(args: string[]): Promise<void> {
  ...
  switch (command) {
    ...
    case 'status': {
      ...
      if (!host) { console.error('Usage: mcutils ping <host> [port] [--json]'); process.exit(1) }
      try {
        const info = await pingServer({ host, port })
        if (jsonFlag) { console.log(JSON.stringify(info, null, 2)); return }
    ...
    case 'rcon': {
      ...
      try {
        ...
        if (jsonFlag) { console.log(JSON.stringify({ command, response: result })); return }
        console.log(result)
      } catch (e: any) { console.error(`Error: ${e.message}`); process.exit(1) }
    ...
    case 'query': {
      ...
      if (!host) { console.error('Usage: mcutils query <host> [port] [--full] [--json]'); process.exit(1) }
      try {
    ...
    case 'server': {
      ...
      if (sub === 'start' && dir) {
        ...
        try {
          ...
          ctrl.on('output', (line: string) => process.stdout.write(line))
    ...
    case 'backup': {
      ...
      if (sub === 'create') {
        ...
        if (!worldDir) { console.error('Usage: mcutils backup create <world-dir> [--out=file.zip]'); process.exit(1) }
        try {
        ...
        const dir = args[2]
        if (!dir) { console.error('Usage: mcutils backup list <dir>'); process.exit(1) }
    ...
    case 'nbt': {
      ...
      if ((sub === 'read' || sub === 'to-json') && args[2]) {
        try {
          ...
          if (jsonFlag) { console.log(json); return }
          console.log(json)
        } catch (e: any) { console.error(`Error: ${e.message}`); process.exit(1) }
      } else {
    ...
    case 'properties': {
      ...
      const file = args[1]
      if (!file) { console.error('Usage: mcutils props <file> [--get key] [--set key=value] [--json]'); process.exit(1) }
      try {
        const props = await ServerProperties.load(file)
        if (args.includes('--get')) {
          const key = args[args.indexOf('--get') + 1]
          const val = props.get(key)
          if (jsonFlag) { console.log(JSON.stringify({ [key]: val })); return }
          console.log(`${key}=${val ?? '(not set)'}`)
        } else if (args.includes('--set')) {
          const kv = args[args.indexOf('--set') + 1]
          const eq = kv?.indexOf('=')
          if (eq === -1 || eq === undefined) { console.error('Usage: --set key=value'); process.exit(1) }
          props.set(kv.slice(0, eq), kv.slice(eq + 1))
          await props.save()
          console.log('Saved.')
        } else {
          if (jsonFlag) { console.log(JSON.stringify(props.toObject(), null, 2)); return }
    ...
    case 'whitelist': {
      ...
      const file = args[1]
      if (!file) { console.error('Usage: mcutils playerlist <file> [--json]'); process.exit(1) }
      try {
        const list = await PlayerListManager.load(file)
        const entries = list.getAll()
        if (jsonFlag) { console.log(JSON.stringify(entries, null, 2)); return }
        console.log(`Total: ${entries.length}`)
        for (const e of entries) console.log(`  ${e.name} (${e.uuid})`)
      } catch (e: any) { console.error(`Error: ${e.message}`); process.exit(1) }
      break
    }
    ...
    case 'level': {
      ...
      const dir = args[1]
      if (!dir) { console.error('Usage: mcutils world <world-dir> [--json]'); process.exit(1) }
      try {
        const level = await readLevelDat(dir)
        if (jsonFlag) { console.log(JSON.stringify(level.info, null, 2)); return }
        const i = level.info
    ...
    case 'uuid': {
      ...
      if (!username) { console.error('Usage: mcutils uuid <username> [--online] [--json]'); process.exit(1) }
      if (args.includes('--online')) {
        const result = await resolveUsernameToUuid(username)
        if (result) {
          if (jsonFlag) { console.log(JSON.stringify(result)); return }
          console.log(`  UUID:     ${result.uuid}`)
          console.log(`  Username: ${result.username}`)
        } else { console.error('Could not resolve UUID'); process.exit(1) }
      } else {
        const u = uuidOffline(username)
        if (jsonFlag) { console.log(JSON.stringify({ uuid: u, username, onlineMode: false })); return }
    ...
    case 'schematic': {
      ...
      const file = args[1]
      if (!file) { console.error('Usage: mcutils schematic <file> [--json]'); process.exit(1) }
      try {
        const schem = await readSchematic(file)
        if (jsonFlag) { console.log(JSON.stringify(schem, null, 2)); return }
    ...
    case 'log': {
      ...
      const file = args[1]
      if (!file) { console.error('Usage: mcutils log <file> [--json]'); process.exit(1) }
      try {
        const parser = new LogParser()
        const events = await parser.parseFile(file)
        const counts: Record<string, number> = {}
        for (const e of events) counts[e.type] = (counts[e.type] ?? 0) + 1
        if (jsonFlag) { console.log(JSON.stringify({ total: events.length, counts, events }, null, 2)); return }
